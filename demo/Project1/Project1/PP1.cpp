...
int main()
{
	...
	start_button_rect.left = (1400 - button_width) / 2;
	...
	exit_button_rect.left = (1400 - button_width) / 2;
	exit_button_rect.top = 400;
	exit_button_rect.right = exit_button_rect.left + button_width;
	exit_button_rect.bottom = exit_button_rect.top + button_height;
	...
	while (IsRunning)
	{
		...
		if (start_game)
		{
			...
			//boss的刷新
			if (s >= 15)trygenerate_boss(b_list), boss_apper = true;
		...
		if (start_game)
		{
			...
			//boss的绘制
			if (boss_apper)for (boss* boss : b_list)boss->draw(1000 / 144);
			//子弹的绘制
			...
			if (boss_apper)for (boss* boss : b_list)
			{
				for (bullet& b : bb)
				{
